#!/usr/bin/env python3
"""Deterministic bo-en raw-bilingual-glossary builder, adapted from
glossary-extract-raw's own regenerate_raw_glossary.py reference script for
this vault's 2-tier (\\gla/\\glb) Tibetan gloss data (that reference script
is hardcoded for a 3-tier Pali \\gla/\\glb/\\glc scheme with Pali-specific
frontmatter and a Pali function-word skiplist, so it is adapted here rather
than run as-is).
...
CSV_PATH = Path("0-INBOX/temp/bo-en-pairs.csv")
GLOSS_PATH = Path("2-RAILS/Bilingual-Glossaries/Raw/bo-en-gloss.md")
OUT_PATH = Path("2-RAILS/Bilingual-Glossaries/Raw/bo-en.md")
MIN_FREQ = 3

# Pure grammatical particles / case markers / connectives / pronouns --
# carry no independent lexical content for a translation-consistency
# glossary (analogous to SKIP_LEMMAS in the Pali reference script).
SKIP_LEMMAS = {
...
lines.append("target_file: 1-SOURCES/Translations/bo-ཟབ་ཏིག་སྒྲོལ་ཆོག-en.md")
lines.append("source_language: bo")
lines.append("target_language: en")
lines.append("language_pair: bo-en")
lines.append("target_lang_tag: en")
