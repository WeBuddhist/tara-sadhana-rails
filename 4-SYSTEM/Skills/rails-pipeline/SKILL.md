...
> **Vault copy.** Canonical source: `Webuddhist-Skills/rails/rails-pipeline/SKILL.md` —
> edit it there and re-copy. In this vault, `rails/CONVENTIONS.md` and `rails/PROFILES.md`
> live in that repo; if it is not available, use `4-SYSTEM/CLAUDE.md` (§2, §5–5b, §6) for the
> same rules and the `rails-vault` profile paths. Skills here sit under `4-SYSTEM/Skills/`,
> mostly under the legacy names mapped at the end of this file; a rails skill with no copy
> in this vault must be loaded from the Webuddhist-Skills repo or reported as unavailable.
